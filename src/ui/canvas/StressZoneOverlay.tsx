 ...
 * "Show Stress Zones" toggle is on.
 *
...
import { COMPONENT_DEFS, COMP_H, COMP_W, type Circuit, type SimulationResult } from '../../domain';
...
export function StressZoneOverlay({ circuit, simulation }: Props) {
  const enabled = useSettingsStore((s) => s.stressZonesEnabled);
  const regulationStandard = useSettingsStore((s) => s.regulationStandard) as StandardId;
  ...
  // so callers (and tests) can detect the toggle is armed.
  if (!simulation) {
    return <g pointerEvents="none" data-stress-zone-overlay />;
  }

  return (
    <g pointerEvents="none" data-stress-zone-overlay style={{ isolation: 'isolate' }}>
      {/* Component heat halos */}
      {components.map((comp) => {
        ...
        return (
          <g key={`stress-c-${comp.id}`}>
            ...
            <g transform={`translate(${comp.x}, ${y - 12})`}>
              ...
              >
                {Math.round(ratio * 100)}%{thermal ? ` · ${Math.round(thermal.temperature)}°C` : ''}
              </text>
            </g>
          </g>
        );
      ...
      {/* Wire stress bands: heat ratio + voltage-drop ceiling. */}
      {wires.map((wire) => {
        ...
        const ceiling = fromComp
          ? voltageDropCeiling(fromComp.type, standard)
          : standard.voltageDrop.powerPercent;
        const dropRatio = calc ? calc.voltageDropPercent / ceiling : 0;
        const ratio = Math.max(heatRatio, dropRatio, wire.isBusted ? 1 : 0);
        if (ratio < 0.25) return null;

        // We don't recompute the path geometry here — the WireLayer already
        // builds it per render. Instead, render a wide translucent band at
        // the wire's bounding midpoint as a stress indicator dot so the
        // overlay is visible without duplicating the router.
