...
- **Structured learning modes (v2).** **Challenge Mode** offers hand-authored build challenges — a protected lamp, a momentary doorbell, an RCBO-fed socket — judged by a rule checklist on the real simulator, with progressive hints and a safe practice workspace that snapshots your normal circuit and restores it exactly. The **Diagnosis Lab** generates an intentional fault for you to find and repair; opt-in **Ohmageddon Mode** adds deliberately harder, electrically honest scenarios. Diagnosis exercises are reproducible from their seed — copy it, paste it, get the identical circuit. Progress and statistics stay in IndexedDB; no accounts, no backend.
...
- **In-app documentation (Phase 6.5.1).** Full-page docs overlay with 7 sections: Getting Started, Components Reference (auto-generated from the registry), Wiring Guide, Keyboard Shortcuts, Simulation & Faults, Learning Modes, Tips & Tricks. Wire-style design, sidebar TOC, scroll-to-section from the menu.
