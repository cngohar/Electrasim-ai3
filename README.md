...
- **Generated learning modes (v2).** Three exercise types built on one deterministic circuit generator: **Challenge Mode** (build a circuit to a target), the **Diagnosis Lab** (find and repair an intentional fault), and opt-in **Ohmageddon Mode** (deliberately harder scenarios that stay electrically honest). Every exercise is reproducible from its seed — copy it, paste it, get the identical circuit. Progress and statistics stay in IndexedDB; no accounts, no backend.
